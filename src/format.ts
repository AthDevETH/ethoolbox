...
import type { Currency, CurrencyAmount, Percentage, Price } from './entities';
import type { Dnum, Nullish } from './types';
...
export function formatPriceImpact(priceImpact: Percentage | undefined) {
    if (!priceImpact) return '-';

    return `${priceImpact.multiply(-1).toFixed(3)}%`;
}

export function formatSlippage(slippage: Percentage | undefined) {
    if (!slippage) return '-';
...
export function toSignificant(
    ...
    const decimalRawAmount = new Decimal(value[0].toString()).div(10 ** value[1]);
    ...
    const quotient = decimalRawAmount.toSignificantDigits(significantDigits);
...
export function toFixed(
    ...
    Decimal.set({ precision: decimalPlaces + 1, rounding: toFixedRounding[rounding] });
    const quotient = decimalRawAmount.toDecimalPlaces(decimalPlaces);

    // Use toFixed method on the quotient object to preserve the specified number of decimal places
    const formattedValue = quotient.toFixed(decimalPlaces);

    const groupSeparator = (format as { groupSeparator?: string }).groupSeparator;
    // Add the group separator if specified
    const formattedValueWithSeparator = groupSeparator
        ? formattedValue.replace(/\B(?=(\d{3})+(?!\d))/g, groupSeparator)
        : formattedValue;

    return formattedValueWithSeparator;
}
